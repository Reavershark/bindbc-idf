module idf.esp_event.esp_event;

import idf.esp_common.esp_err : esp_err_t;
import idf.esp_event.esp_event_base;
import idf.freertos : BaseType_t, FILE, TickType_t, UBaseType_t;
...
{
    int queue_size;            /// Size of the event loop queue
    const char* task_name;     /** Name of the event loop task \
                                   If NULL, a dedicated task is not created for event loop */
    UBaseType_t task_priority; /// Priority of the event loop task, ignored if task name is NULL
    uint task_stack_size;      /// Stack size of the event loop task, ignored if task name is NULL
    BaseType_t task_core_id;   /// Core to which the event loop task is pinned to, ignored if task name is NULL
}
// dfmt on
 ...
 *  - Others: Fail
 */
esp_err_t esp_event_loop_create(
    const esp_event_loop_args_t* event_loop_args, esp_event_loop_handle_t* event_loop
);
...
esp_err_t esp_event_handler_register(
    esp_event_base_t event_base, int event_id,
...
static if (is(typeof(CONFIG_ESP_EVENT_POST_FROM_ISR)))
{
    /**
     ...
     *  - Others: Fail
     */
    esp_err_t esp_event_isr_post_to(
        esp_event_loop_handle_t event_loop,
        esp_event_base_t event_base, int event_id,
        void* event_data, size_t event_data_size,
        BaseType_t* task_unblocked,
