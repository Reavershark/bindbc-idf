module idf.esp_hw_support.esp_intr_alloc;
...
enum ESP_INTR_FLAG_LEVEL1       = 1 << 1;  ///< Accept a Level 1 interrupt vector (lowest priority)
enum ESP_INTR_FLAG_LEVEL2       = 1 << 2;  ///< Accept a Level 2 interrupt vector
enum ESP_INTR_FLAG_LEVEL3       = 1 << 3;  ///< Accept a Level 3 interrupt vector
enum ESP_INTR_FLAG_LEVEL4       = 1 << 4;  ///< Accept a Level 4 interrupt vector
enum ESP_INTR_FLAG_LEVEL5       = 1 << 5;  ///< Accept a Level 5 interrupt vector
enum ESP_INTR_FLAG_LEVEL6       = 1 << 6;  ///< Accept a Level 6 interrupt vector
enum ESP_INTR_FLAG_NMI          = 1 << 7;  ///< Accept a Level 7 interrupt vector (highest priority)
enum ESP_INTR_FLAG_SHARED       = 1 << 8;  ///< Interrupt can be shared between ISRs
enum ESP_INTR_FLAG_EDGE         = 1 << 9;  ///< Edge-triggered interrupt
enum ESP_INTR_FLAG_IRAM         = 1 << 10; ///< ISR can be called if cache is disabled
enum ESP_INTR_FLAG_INTRDISABLED = 1 << 11; ///< Return with this interrupt disabled

/// Low and medium prio interrupts. These can be handled in C.
enum ESP_INTR_FLAG_LOWMED = ESP_INTR_FLAG_LEVEL1 | ESP_INTR_FLAG_LEVEL2 | ESP_INTR_FLAG_LEVEL3;
/// High level interrupts. Need to be handled in assembly.
enum ESP_INTR_FLAG_HIGH = ESP_INTR_FLAG_LEVEL4 | ESP_INTR_FLAG_LEVEL5 | ESP_INTR_FLAG_LEVEL6 | ESP_INTR_FLAG_NMI;

/// Mask for all level flags
enum ESP_INTR_FLAG_LEVELMASK = ESP_INTR_FLAG_LEVEL1 | ESP_INTR_FLAG_LEVEL2 | ESP_INTR_FLAG_LEVEL3
 ...
 */
enum ETS_INTERNAL_TIMER0_INTR_SOURCE    =  -1; /// Platform timer 0 interrupt source
enum ETS_INTERNAL_TIMER1_INTR_SOURCE    =  -2; /// Platform timer 1 interrupt source
enum ETS_INTERNAL_TIMER2_INTR_SOURCE    =  -3; /// Platform timer 2 interrupt source
enum ETS_INTERNAL_SW0_INTR_SOURCE       =  -4; /// Software int source 1
enum ETS_INTERNAL_SW1_INTR_SOURCE       =  -5; /// Software int source 2
enum ETS_INTERNAL_PROFILING_INTR_SOURCE =  -6; /// Int source for profiling
enum ETS_INTERNAL_UNUSED_INTR_SOURCE    = -99; /// Interrupt is not assigned to any source

/// Provides SystemView with positive IRQ IDs, otherwise scheduler events are not shown properly
enum ETS_INTERNAL_INTR_SOURCE_OFF = -ETS_INTERNAL_PROFILING_INTR_SOURCE;

/// Enable interrupt by interrupt number
alias ESP_INTR_ENABLE = esp_intr_enable_source;

/// Disable interrupt by interrupt number
alias ESP_INTR_DISABLE = esp_intr_disable_source;

/// Function prototype for interrupt handler function
alias intr_handler_t = void function(void *arg);

/// Interrupt handler associated data structure
struct intr_handle_data_t;

/// Handle to an interrupt handler
alias intr_handle_t = intr_handle_data_t*;
 ...
 *         ESP_OK otherwise
 */
esp_err_t esp_intr_alloc_intrstatus(int source, int flags, uint intrstatusreg, uint intrstatusmask, intr_handler_t handler, void* arg, intr_handle_t* ret_handle);
